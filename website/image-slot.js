...
(() => {
  ...
  class ImageSlot extends HTMLElement {
    static get observedAttributes() {
      return ['shape', 'radius', 'mask', 'fit', 'placeholder', 'src', 'id', 'credit', 'credit-href', 'position', 'align'];
    }
    ...
    _applyView() {
      ...
      if (!g) {
        ...
        this._img.style.objectPosition = 'top center';
        return;
      }
      ...
      const pos = (this.getAttribute('position') || this.getAttribute('align') || 'top').toLowerCase();
      const isTop = pos !== 'center' && pos !== 'bottom';
      
      let tVal = 50 + this._view.y;
      if (this._view.y === 0 && isTop && numH > 100) {
        // When taller than frame and not manually panned, anchor top edge at 0%
        // (tVal - numH/2 = 0) so model heads, necklines, and jewelry are fully visible.
        tVal = numH / 2;
      }
      const t = tVal + '%';
      ...
      if (this.hasAttribute('data-reframe')) {
        ...
        this._spill.style.top = (r.top + tVal / 100 * r.height) + 'px';
      }
    }
